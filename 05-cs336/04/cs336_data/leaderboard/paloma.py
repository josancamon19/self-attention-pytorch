...
print(f"  Short docs (<500 chars): {sum(1 for x in doc_lengths if x < 500)} ({sum(1 for x in doc_lengths if x < 500)/len(doc_lengths)*100:.1f}%)")
print(f"  Long docs (>5000 chars): {sum(1 for x in doc_lengths if x > 5000)} ({sum(1 for x in doc_lengths if x > 5000)/len(doc_lengths)*100:.1f}%)")
...
if readability_scores:
    ...
    print(f"  Very easy (90-100): {sum(1 for x in readability_scores if x >= 90)} ({sum(1 for x in readability_scores if x >= 90)/len(readability_scores)*100:.1f}%)")
    print(f"  Easy (80-90): {sum(1 for x in readability_scores if 80 <= x < 90)} ({sum(1 for x in readability_scores if 80 <= x < 90)/len(readability_scores)*100:.1f}%)")
    print(f"  Standard (70-80): {sum(1 for x in readability_scores if 70 <= x < 80)} ({sum(1 for x in readability_scores if 70 <= x < 80)/len(readability_scores)*100:.1f}%)")
    print(f"  Difficult (<70): {sum(1 for x in readability_scores if x < 70)} ({sum(1 for x in readability_scores if x < 70)/len(readability_scores)*100:.1f}%)")
...
patterns = {
    ...
    'very_short_sentences': sum(1 for doc in documents if len([s for s in re.split(r'[.!?]+', doc) if len(s.strip()) < 10]) / max(1, len(re.split(r'[.!?]+', doc))) > 0.5),
...
spam_indicators = ['click', 'here', 'buy', 'now', 'free', 'offer', 'deal', 'sale', 'discount', 'limited', 'time', 'call', 'contact', 'email', 'website', 'link', 'follow', 'subscribe', 'join']
