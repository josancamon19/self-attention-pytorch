...
def test_gpt2(model_size="gpt2"):
    ...
    openai_outputs = openai_model(
        input_ids=sent_ids, attention_mask=att_mask, output_hidden_states=True
    ).hidden_states[-1]
    ...
    # Checks to validate assertion allclose
    print(
        f"Max absolute difference: {torch.max(torch.abs(outputs['last_hidden_state'] - openai_outputs))}"
    )
    print(
        f"Mean absolute difference: {torch.mean(torch.abs(outputs['last_hidden_state'] - openai_outputs))}"
    )
    print(f"outputs sample: {outputs['last_hidden_state'][0, 0, :5]}")
    print(f"OpenAI outputs sample: {openai_outputs[0, 0, :5]}")
